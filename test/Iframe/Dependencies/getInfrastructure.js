...
describe('createQuery()', function() {
    it('should create an overpass query string from a list of queries and a bounds string', function() {
        assert.deepEqual(getInfrastrucuture.createQuery([{query:"waterway=dam"},{query:"waterway=river"}],"61,31,60,30"),"node[waterway=dam](61,31,60,30);way[waterway=dam](61,31,60,30);relation[waterway=dam](61,31,60,30);node[waterway=river](61,31,60,30);way[waterway=river](61,31,60,30);relation[waterway=river](61,31,60,30);");
    });
});
...
describe('withinBounds', function() {
    it('should return true if input bounds are within global object "currentBounds," should return false if null or not within', function() {
        ...
        getInfrastrucuture.currentBounds = getInfrastrucuture.changeCurrentBounds({north:60,south:61,east:30,west:31});
        assert.deepEqual(getInfrastrucuture.withinBounds({north:59,south:62,east:29,west:32}),true); //within case
        assert.deepEqual(getInfrastrucuture.withinBounds({north:61,south:60,east:31,west:30}),false); //not within case
    });
});
/*
